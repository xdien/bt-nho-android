...
public class HangHoaAdapter extends BaseAdapter {
    Context context;
    ArrayList<HangHoa> dsHangHoas;
    SharedPreferences caiDat;
    SharedPreferences.Editor caiDatEditor;
    String path;

    public HangHoaAdapter(Context context, ArrayList<HangHoa> dsHangHoas) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        //load tu duong dan
        File imgFile = new File(path + "/"+ dsHangHoas.get(position).getHinhDuongDan());
        if(imgFile.exists()){
            Bitmap myBitmap = BitmapFactory.decodeFile(imgFile.getAbsolutePath());
            hangHoaItem.imageView.setImageBitmap(myBitmap);
        }
        hangHoaItem.tenTextView.setText(dsHangHoas.get(position).getTen());
